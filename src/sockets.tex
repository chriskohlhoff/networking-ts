...
and \tcode{s} denotes a (possibly const) value of a type
...
\pnum
In the table below, \tcode{a} denotes a (possibly const) value of type \tcode{X},
\tcode{p} denotes a (possibly const) value of a type
that meets the \tcode{Protocol}~(\ref{socket.reqmts.protocol}) requirements,
and \tcode{u} denotes an identifier.
...
\pnum
In the table below, \tcode{a} denotes a (possibly const) value of type \tcode{X},
\tcode{v} denotes a (possibly const) value of type \tcode{bool},
and \tcode{u} denotes an identifier.
...
\pnum
In the table below, \tcode{a} denotes a (possibly const) value of type \tcode{X},
\tcode{v} denotes a (possibly const) value of type \tcode{int},
and \tcode{u} denotes an identifier.
...
\tcode{ec} denotes a (possibly const) value of type \tcode{error_code},
and \tcode{ep} denotes a (possibly const) value of a type satisfying
...
\pnum
\tcode{linger} satisfies the requirements of \tcode{Destructible} (\CppXref{destructible}), \tcode{DefaultConstructible} (\CppXref{defaultconstructible}), \tcode{CopyConstructible} (\CppXref{copyconstructible}), \tcode{CopyAssignable} (\CppXref{copyassignable}), \tcode{Gettable\-Socket\-Option}~(\ref{socket.reqmts.gettablesocketoption}), and \tcode{SettableSocketOption}~(\ref{socket.reqmts.settablesocketoption}).
...
\pnum
Class template \tcode{basic_socket<Protocol>} is used as the base class for the \tcode{basic_datagram_socket<Pro\-t\-o\-col>} and \tcode{basic_stream_socket<Protocol>} class templates. It provides functionality that is common to both types of socket.
...
inline namespace v1 {
  ...
  public:
    ...
    using native_handle_type = @\impdefx{type of \tcode{basic_socket::native_handle_type}}@; // \nativeref
...
\begin{itemize}
\item
\tcode{addressof(context()) == addressof(ctx)}.
\item
...
\begin{itemize}
\item
\tcode{addressof(context()) == addressof(rhs.context())}.
\item
...
inline namespace v1 {
  ...
  public:
    ...
    using native_handle_type = @\impdefx{type of \tcode{basic_datagram_socket::native_handle_type}}@; // \nativeref
    using protocol_type = Protocol;
    using endpoint_type = typename protocol_type::endpoint;
...
inline namespace v1 {
  ...
  public:
    ...
    using native_handle_type = @\impdefx{type of \tcode{basic_stream_socket::native_handle_type}}@; // \nativeref
    using protocol_type = Protocol;
    using endpoint_type = typename protocol_type::endpoint;
...
\pnum
Instances of class template \tcode{basic_stream_socket} meet the requirements of \tcode{Destructible} (\CppXref{destructible}), \tcode{MoveConstructible} (\CppXref{moveconstructible}), \tcode{MoveAssignable} (\CppXref{moveassignable}), \tcode{SyncRead\-Stream}~(\ref{buffer.stream.reqmts.syncreadstream}), \tcode{SyncWriteStream}~(\ref{buffer.stream.reqmts.syncwritestream}), \tcode{AsyncReadStream}~(\ref{buffer.stream.reqmts.asyncreadstream}), and \tcode{AsyncWriteStream}~(\ref{buffer.stream.reqmts.asyncwritestream}).
...
inline namespace v1 {
  ...
  public:
    ...
    using executor_type = io_context::executor_type;
    using native_handle_type = @\impdefx{type of \tcode{basic_socket_acceptor::native_handle_type}}@; // \nativeref
...
\begin{itemdescr}
\pnum
\returns \tcode{accept(context(), ec)}.
\end{itemdescr}
...
\returns
\begin{codeblock}
async_accept(context(), forward<CompletionToken>(token))
\end{codeblock}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{accept(context(), endpoint, ec)}.
\end{itemdescr}
...
\returns
\begin{codeblock}
async_accept(context(), endpoint, forward<CompletionToken>(token))
\end{codeblock}
\end{itemdescr}

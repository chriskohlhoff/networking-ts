...
\item
\tcode{execution::outstanding_work.untracked}
\item
...
\pnum
\tcode{io_context} executors having a different set of established properties are represented by a distinct, unspecified type.
...
\pnum
\placeholder{C} is a type satisfying the OneWayExecutor requirements~(P0443R7). Objects of type \placeholder{C} are associated with an \tcode{io_context}, and function objects submitted using the \tcode{execute} member function will be executed by the \tcode{io_context} from within a run function.

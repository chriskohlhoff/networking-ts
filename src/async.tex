...
inline namespace v1 {
  ...
  using executor = execution::executor<
    ...
    execution::prefer_only<execution::not_continuation>>;
...
\pnum
\begin{note} The function \tcode{dispatch} satisfies the requirements for an asynchronous operation~(\ref{async.reqmts.async}), except for the requirement that the operation uses the \tcode{execution::never_blocking} property if it completes immediately. \end{note}
...
\begin{itemize}
\item
an executor object \tcode{w} for the completion handler's associated executor, initialized with \tcode{execution::prefer(get_associated_executor(h), execution::outstanding_work)}
\item
...
inline namespace v1 {
  ...
  public:
    ...
    template<class Property>
      @\seebelow@ query(const Property& p) const;
...
\item
a function object \tcode{f1} submitted to the strand \tcode{s1} when the \tcode{execution::never_blocking} property is established in \tcode{s1}, or when \tcode{s1.running_in_this_thread()} is \tcode{false}

\item
a function object \tcode{f2} submitted to the strand \tcode{s2} when the \tcode{execution::never_blocking} property is established in \tcode{s2}, or when \tcode{s2.running_in_this_thread()} is \tcode{false}
\end{itemize}

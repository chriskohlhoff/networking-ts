...
\pnum
An asynchronous operation has an associated executor satisfying the general requirements on executors (P0443R7). If not otherwise specified by the asynchronous operation, this associated executor is an object of type \tcode{system_executor}.
...
satisfying the general requirements on executors (P0443R7).
...
\begin{itemize}
\item
an executor object \tcode{work1}, initialized as \tcode{execution::prefer(ex1, execution::outstanding_work.tracked)}; and

\item
an executor object \tcode{work2}, initialized as \tcode{execution::prefer(ex2, execution::outstanding_work.tracked)}.
\end{itemize}
...
\item
an object of an unspecified type satisfying the general requirements on executors (P0443R7), that delegates executor operations to the type \tcode{Executor2} and object \tcode{ex2}.
\end{itemize}
...
\pnum
The \tcode{executor_arg_t} struct is an empty structure type used as a unique type to disambiguate constructor and function overloading. Specifically, types may have constructors with \tcode{executor_arg_t} as the first argument, immediately followed by an argument of a type that satisfies the general requirements on executors (P0443R7).
...
an executor of type \tcode{Executor} satisfying the general requirements on executors (P0443R7)
...
\pnum
\placeholder{C} is a type satisfying the OneWayExecutor requirements~(P0443R7).
...
\begin{itemdescr}
\pnum
\returns \tcode{execution::mapping.thread}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns The established value of the property for the executor type \placeholder{C}.
\end{itemdescr}
...
\pnum
\begin{note} The function \tcode{dispatch} satisfies the requirements for an asynchronous operation~(\ref{async.reqmts.async}), except for the requirement that the operation uses the \tcode{execution::blocking.never} property if it completes immediately. \end{note}
...
\end{codeblock}
where \tcode{ex} is the result of \tcode{get_associated_executor(completion.completion_handler)}, and \tcode{alloc} is the result of \tcode{get_associated_allocator(completion.completion_handler)}.
\end{itemize}
...
\begin{itemize}
\item
an executor object \tcode{w} for the completion handler's associated executor, initialized with \tcode{execution::prefer(get_associated_executor(h), execution::outstanding_work.tracked)}
\item
...
\end{codeblock}
where \tcode{alloc} is the result of \tcode{get_associated_allocator(h)}.
\item
Performs:
\begin{codeblock}
execution::prefer(
...
\pnum
\begin{note} The function \tcode{defer} satisfies the requirements for an asynchronous operation~(\ref{async.reqmts.async}). \end{note}
...
\pnum
The class template \tcode{strand} is a wrapper around an object of type \tcode{Executor} satisfying the OneWayExecutor requirements (P0443R7).
...
inline namespace v1 {
  ...
  public:
    ...
    template<class Property>
      @\seebelow@ require(const Property& p) const;

    template<class Property>
      static constexpr @\seebelow@ query(const Property& p);

    template<class Property>
      constexpr @\seebelow@ query(const Property& p) const;
...
\item
a function object \tcode{f1} submitted to the strand \tcode{s1} when the \tcode{execution::blocking.never} property is established in \tcode{s1}, or when \tcode{s1.running_in_this_thread()} is \tcode{false}

\item
a function object \tcode{f2} submitted to the strand \tcode{s2} when the \tcode{execution::blocking.never} property is established in \tcode{s2}, or when \tcode{s2.running_in_this_thread()} is \tcode{false}
\end{itemize}
...
\begin{itemdescr}
\pnum
\returns \tcode{true} if the current thread of execution is running a function that was submitted to the strand, or to any other strand object that shares the same ordered, non-concurrent state, using \tcode{execute}; otherwise \tcode{false}. \begin{note} That is, the current thread of execution's call chain includes a function that was submitted to the strand. \end{note}
\end{itemdescr}
...
\pnum
\returns A strand \tcode{s} of type \tcode{strand<decay_t<decltype(inner_ex_.require(p))>>}, where \tcode{s.inner_ex_} is initialized with \tcode{inner_ex_.require(p)}, and sharing the same ordered, non-concurrent state as \tcode{*this}.

\pnum
\remarks Shall not participate in overload resolution unless \tcode{inner_ex_.require(p)} is well-formed.
...
\begin{itemdescr}
\pnum
\returns \tcode{Executor::query(p)}.

\pnum
\remarks Shall not participate in overload resolution unless \tcode{Executor::query(p)} is well-formed.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{inner_ex_.query(p)}.

\pnum
\remarks Shall not participate in overload resolution unless \tcode{inner_ex_.query(p)} is well-formed.
\end{itemdescr}

\indexlibrarymember{execute}{strand}%
\begin{itemdecl}
template<class Function>
  void execute(Function&& f) const;
\end{itemdecl}
...
\begin{itemdescr}
\pnum
\effects Submits \tcode{f} to the executor \tcode{inner_ex_}, such that the guarantees of ordering and non-concurrency are met.
\end{itemdescr}
...
\pnum
For any executor type \tcode{E}, the associated object for the associator \tcode{associated_executor<H, E>} is an executor where, for function objects executed using the executor's \tcode{execute()} function, any exception thrown is caught by a function object and stored in the associated shared state.
\end{itemdescr}
...
\pnum
The implementation specializes \tcode{associated_executor} for \tcode{F}. For function objects executed using the associated executor's \tcode{execute()} function, any exception thrown is caught by the executor and stored in the associated shared state.

\pnum
For any executor type \tcode{E}, the associated object for the associator \tcode{associated_executor<F, E>} is an executor where, for function objects executed using the executor's \tcode{execute()} function, any exception thrown by a function object is caught by the executor and stored in the associated shared state.

...
\pnum
In the table below,
\tcode{a} and \tcode{b} denote (possibly const) values of type \tcode{X}.
...
\pnum
In the table below,
\tcode{a} denotes a (possibly const) value of type \tcode{address},
\tcode{b} and \tcode{c} denote (possibly const) values of type \tcode{address_v4},
\tcode{d} denotes a (possibly const) value of type \tcode{address_v6},
\tcode{e} denotes a (possibly const) value of type \tcode{unsigned int},
...
\begin{itemdescr}
\pnum
\returns If \tcode{is_host()} is \tcode{true}, an \tcode{address_v6_range} object representing the single address \tcode{this->add\-r\-ess()}. Otherwise, an \tcode{address_v6_range} object representing the range of unique host IP addresses in the network.
\end{itemdescr}
...
\begin{itemize}
\item
If \tcode{proto == protocol_type::v6()}, \tcode{this->address() == ip::address_v6()}; otherwise,\\
\tcode{this->address() == ip::address_v4()}.
\item
\tcode{port() == port_num}.
\end{itemize}
\end{itemdescr}
...
let \tcode{p} denote a (possibly const) value of a type meeting the protocol~(\ref{socket.reqmts.protocol}) requirements,
